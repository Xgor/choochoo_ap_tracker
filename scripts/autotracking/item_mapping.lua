BASE_LOCATION_ID = 66600000
ITEM_MAPPING = {
	[BASE_ITEM_ID + 6] = { "south_mine_key" },
	[BASE_ITEM_ID + 7] = { "north_mine_key" },
	[BASE_ITEM_ID + 8] = { "mountain_ruin_key" },
	[BASE_ITEM_ID + 9] = { "barn_key" },
	[BASE_ITEM_ID + 10] = { "candice_key" },
	[BASE_ITEM_ID + 11] = { "dead_fish" },
	[BASE_ITEM_ID + 12] = { "lockpicks" },
	[BASE_ITEM_ID + 13] = { "tablet" },
	[BASE_ITEM_ID + 14] = { "blue_box" },
	[BASE_ITEM_ID + 15] = { "page" },
	[BASE_ITEM_ID + 16] = { "journal" },
	[BASE_ITEM_ID + 17] = { "dynamite" },
	[BASE_ITEM_ID + 18] = { "rockets" },
	[BASE_ITEM_ID + 19] = { "breaker" },
	[BASE_ITEM_ID + 20] = { "bob" },
	[BASE_ITEM_ID + 21] = { "contracts" },
    [BASE_ITEM_ID + 22] = { "mob_camp_key" },
	[BASE_ITEM_ID + 23] = { "pickles" },

	[BASE_ITEM_ID + 37] = { "green_egg" },
	[BASE_ITEM_ID + 38] = { "blue_egg" },
	[BASE_ITEM_ID + 39] = { "red_egg" },
	[BASE_ITEM_ID + 40] = { "explosive" },
	[BASE_ITEM_ID + 41] = { "explosive" }, -- TODO impement x8 explosive
	[BASE_ITEM_ID + 42] = { "temple_key" },
	[BASE_ITEM_ID + 43] = { "bug_spray" },

	-- multiple items on this id, add the consumable 3 times
	-- [BASE_ITEM_ID + 00004] = { { "toggle" }, { "consumable", nil, 3 } }
}